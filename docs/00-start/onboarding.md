> **Status:** canonical
> **SSoT kode:** syilex/ · syilex-frontend/ · .env.example
> **Jika konflik:** ikuti kode, lalu update dokumen ini.
# Onboarding — Dev Baru POSIP

> **Monorepo:** kode di `syilex/` + `syilex-frontend`. Docs hub: [`../README.md`](../README.md). Agent: [`../ai/README.md`](../ai/README.md).
...
Buat database `posip` (atau nama di `.env`) di HeidiSQL / phpMyAdmin.
...
**Catatan:** Laragon otomatis bikin virtual host dari nama folder — `C:/laragon/www/POSIP/syilex` → `http://localhost:8000`. Kalau tidak, klik kanan Laragon tray → Menu → Apache/Nginx → Reload.
...
| Email | Password | Role |
|-------|----------|------|
| super-admin@posip.com | password | super-admin (all permissions) |
| admin@posip.com | password | admin |
| kasir@posip.com | password | kasir |
...
1. Buka [PosKasirPage.vue](../../syilex-frontend/src/views/pos/PosKasirPage.vue), cari function `onPayment()` (sekitar baris 500-600)
...
### Domain POS
- Baca [`../ai/README.md`](../ai/README.md) untuk business rules kritis
...
- Pertanyaan kode: cek [`../ai/README.md`](../ai/README.md) dulu, lalu tanya tim via Slack/WA
