> **Status:** canonical
> **SSoT kode:** syilex-frontend/tests/
...
| `printAdapter.test.mjs` | Facade browser-only | Empty/invalid payload, write fail, needPicker |
...
| `printPolicy.test.mjs` | Kebijakan PosKasir (static) | Hanya `auto_print_receipt`, openDrawer di encoder, reconnect dialog, no legacy `:5123` |
| `deadCodeGuard.test.mjs` | Leftover Sakai/unused files stay deleted | BlockViewer, EmptyState, types/index |
| `masterUiGuard.test.mjs` | Master UI unify policy | RowActionButtons on Aksi columns, text not outlined, Produk ImageUpload, Warehouse filter badge |
...
**Catatan:** E2E tidak menguji cetak thermal browser (tidak ada printer di CI). Thermal diverifikasi unit + smoke manual (pairing Chrome di POS Terminal, test print, checkout auto-print, PDF di Firefox).
...
Checklist: pairing Chrome di POS Terminal, test print terminal, checkout auto-print, PDF di Firefox.
