...
| SO-B4 | P0 | **Serial hydrate:** save diblok selama `present === null`. Bug: `emitIfChanged` menyamakan `null` dengan `[]` → SKU serial **tanpa unit tersedia** tidak pernah emit; Simpan mati meski SN lain sudah dicentang. **FIXED:** picker emit `[]` setelah load bila parent masih `null`. Form Full auto-loop halaman `all-products`. Guard `null` tetap (anti mass-hilang). | `SerialUnitPicker` emitIfChanged; Form `loadAllProducts` | — |
...
| SO-B9 | P1 | **Ganti gudang/mode di draft:** FE enable; ganti field mereset detail (konfirmasi). BE cek 1 draft/WH (exclude dokumen ini). Edit Full: `allProductsLoaded=true` supaya tidak append halaman → duplikat produk. | Update Action; Form loadOpname + Select gudang/mode | — |
