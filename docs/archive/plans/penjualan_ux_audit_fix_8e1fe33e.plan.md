...
overview: "Rencana perbaikan tanpa ubah kode dulu: tempo↔cash dua arah, filter promo overlap, tombol Get Promo (auto menang), faktur PDF khusus, quick-add produk di baris, list mobile, dan lengkungan login — reuse komponen existing, anti N+1, diff seminimal mungkin (ponytail)."
todos: []
isProject: false
---
...
|------|----------|------------|
| Tempo dan Cash | Bisa bebas (tempo 0 tanpa cash, cash tanpa tempo 0) | Dua arah saling kunci |
...
**Edge case**
- Ganti customer → isi `tempo_default`; jika 0 → paksa cash; jika >0 dan cash sedang ON → ikuti aturan cash (cash tetap = tempo 0)
...
**Sebab:** backend containment di [PromoController.php](syilex/app/Http/Controllers/Api/V1/PromoController.php) sekitar L51–60.
...
**Tampil UI:** badge “Promo: …” di baris; slot 1–4 setelah Get Promo jelas bertanda otomatis (boleh masih editable untuk eksperimen, tapi approve menimpa — teks singkat di UI).

**Anti N+1:** reuse `PromoService` / `ManualSalesCalculationService` existing — jangan query promo per line di controller baru.
...
**Baru:** helper khusus (mis. [useExportPdf.js](syilex-frontend/src/composables/useExportPdf.js) sibling `useSalesInvoicePdf.js`) — **khusus penjualan manual**.
...
- Optional: baris “Nomor PO” dari `notes` jika diisi — tanpa kolom DB baru
...
- Piutang unpaid/partial → watermark TEMPO / BELUM LUNAS (konsisten struk online)
...
**Pola reuse:** mirror [CustomerFormDialog.vue](syilex-frontend/src/components/common/CustomerFormDialog.vue) → buat **`ProdukFormDialog.vue`** dari logic Dialog di [ProdukPage.vue](syilex-frontend/src/views/master/ProdukPage.vue) (jangan duplikasi form penuh 2x).
...
2. Di viewport sempit: **matikan frozen** ATAU hanya icon; data tetap scroll horizontal tanpa aksi makan 40% layar.
...
- Hapus / set `border-radius: 0` (atau radius kecil konsisten).
- Cek dark/light tetap rapi; jangan sentuh brand panel kecuali perlu.

---

## Urutan kerja (aman dan cepat)
...
- **Ponytail:** reuse dulu; jangan invent factory; tandai shortcut dengan `ponytail:` jika sadar batasan.
- **UI:** product-ui-design / office-web — restrained; ikon aksi tenang.
- **DB/API:** eager-load; 1 calculate dengan rebuild; jangan N+1 di Get Promo.
