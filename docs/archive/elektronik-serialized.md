> **Status:** archived
> **SSoT kode:** superseded by docs/domain/serial.md
...
battery_cycle_count INT         NULLABLE           -- cycle count
...
- **Nego serial** pakai `promo.allow_manual_discount` (**sudah ada, default `true`**) + cap `promo.max_manual_discount_percent`/`_nominal`. ⚠️ **Jangan set `promo.enabled=false`** di instance elektronik — itu juga mematikan diskon manual ([CheckoutSalesAction:433](../../syilex/app/Actions/Sales/CheckoutSalesAction.php)).
...
| **Stock Card (§2C)** | Tetap 1 entry per gerakan unit (+`product_unit_id`, cost=modal). **Pakai `StockCard::$skipObserver`** saat ubah `inventory_stock.qty` (AI-AGENT.md §7) agar Observer tak buat ADJUSTMENT ganda. |
...
```json
{
  ...
  "units": [{
    ...
    "grade": "A", "battery_health": 92, "battery_cycle_count": 142,
...
Acuan: [CheckoutSalesAction.php](../../syilex/app/Actions/Sales/CheckoutSalesAction.php) (alur penuh terverifikasi), [usePosCart.js](../../../sipos-frontend/src/composables/usePosCart.js).
...
- **SKIP blok restore-HPP** ([:182-189](../../syilex/app/Actions/Sales/ProcessSalesReturnAction.php)) — blok `if (avg_cost==0 && hpp_sales>0) { product.avg_cost=modal; syncAvgCostToInventoryStocks(); }` untuk produk serial (avg selalu 0) **akan mencemari** avg_cost & semua inventory_stock dgn modal 1 unit. Serial: **jangan sentuh avg_cost**.
...
- **Factory/fixture test:** `MasterProdukFactory` + `tracking_type='serial'`; `ProductUnit`/`DocPembelianUnit` via `::create()` manual (AI-AGENT.md §4).
...
- **Kolom template import unit:** `kode_produk` (model), `serial_number`, `grade`, `battery_health`, `battery_cycle_count`, `battery_condition`, `activation_lock`, `kelengkapan`, `modal`, `harga_jual`, `warehouse`, `garansi_sampai`.
