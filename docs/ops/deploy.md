> **Status:** canonical
> **SSoT kode:** syilex/ · deploy scripts · routes/console.php
...
```
* * * * * cd /path/ke/POSIP/syilex && php artisan schedule:run >> /dev/null 2>&1
```
...
### Scheduled Tasks (Artisan Schedule + OS Cron)

Scheduler POSIP:

- **Price Change auto-apply:** Artisan `price-change:apply` dijadwalkan di `routes/console.php` (setiap 5 menit, `withoutOverlapping`). Production **butuh** cron `php artisan schedule:run`.
- **Activity log cleanup:** middleware `CleanupActivityLog` (cooldown panjang) — terpisah dari price change.

Config via database setting (`scheduler.price_change_enabled`, `scheduler.price_change_max_batch`, `scheduler.activity_log_*`).
