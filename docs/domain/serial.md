> **Status:** canonical
> **SSoT kode:** syilex/app/Actions/Serial* · SerialIntake · syilex-frontend serial views
...
> Blueprint lama (ditolak): [`../archive/elektronik-serialized.md`](../archive/elektronik-serialized.md).
> Ringkas untuk agent: [`../ai/business-rules.md`](../ai/business-rules.md) §G.

## TOC

1. Konsep Inti
2. Master Produk — `is_serial`
3. Data Layer
4. Alur Pembelian Serial
5+ (lihat heading di bawah)
...
- File: [MasterProduk.php](../../syilex/app/Models/MasterProduk.php), [ProdukController.php](../../syilex/app/Http/Controllers/Api/V1/ProdukController.php), migrasi `*_add_is_serial_to_master_produk.php`.
...
- **`harga_modal`** = harga beli kotor (input); **`cost_per_unit`** = landed cost (modal + alokasi biaya/pajak header; **diskon header tidak** menurunkan HPP) → dipakai HPP weighted-avg saat approve & basis laporan laba Fase 5.
- **Identitas unit = `kode_internal`** — UNIQUE **global** (index DB), auto-generate `KI-{id}` saat kosong (hook model `created`), **boleh override** (divalidasi unik, `withTrashed`). Migrasi `2026_06_17_100001` + backfill `KI-{id}`. **Override berpola `KI-<angka>` ditolak** (dicadangkan untuk auto-generate → cegah tabrakan dgn kode auto unit lain di masa depan), validasi di [HandlesSerialUnits](../../syilex/app/Actions/SerialIntake/Concerns/HandlesSerialUnits.php).
  ...
  - **Tampil di semua surface:** detail+PDF Pembelian/Perubahan/Koreksi-HPP **+ Adjustment/Transfer/Retur-Beli/Opname** (detail dokumen me-resolve `serial_unit_ids` → unit via trait [AttachesSerialUnitsToDocDetails](../../syilex/app/Http/Controllers/Concerns/AttachesSerialUnitsToDocDetails.php); Adjustment ikut tampil `fate` rusak/hilang), form unit, Register (+filter status rusak/hilang/retur/pending), label, nota POS/online/per-nota/closing, Excel Register + Excel Penjualan per Nota.
...
- **Atribut kondisi (elektronik bekas):** `grade` (A–F), `battery_condition` (Original/Replacement/Service Center/Refurbished), `battery_health` decimal(5,2) %, `battery_cycle_count` (integer ≥ 0), `account_status` (locked/unlocked).
- Model: [SerialUnit.php](../../syilex/app/Models/SerialUnit.php). Scope: `tersedia()`, `terjual()`, `byProduct()`.
...
- Model: [DocSerialIntake.php](../../syilex/app/Models/DocSerialIntake.php). Prefix `PBS` di `SettingService::getPrefix()` + `getPrefixesWithInfo()`.
...
- **HPP weighted-average** dihitung dari **landed cost** `sum(unit.cost_per_unit)/N` → `MasterProduk::recalculateAvgCost(N, avgBatchCost)` (pola persis [ApprovePurchaseOrderAction](../../syilex/app/Actions/PurchaseOrder/ApprovePurchaseOrderAction.php)).
...
- **Edit/Hapus hanya saat draft.** **Approve anti-balapan:** header di-`lockForUpdate()` + cek ulang `isDraft()` **di dalam** transaksi → dua request approve paralel (double-click/retry) → yang kedua ditolak (tak ada dobel stok/hutang/pelunasan). Pola sama di [ApprovePurchaseOrderAction](../../syilex/app/Actions/PurchaseOrder/ApprovePurchaseOrderAction.php). Delete draft → `forceDelete` unit pending.
- Actions: [CreateSerialIntakeAction](../../syilex/app/Actions/SerialIntake/CreateSerialIntakeAction.php) · [ApproveSerialIntakeAction](../../syilex/app/Actions/SerialIntake/ApproveSerialIntakeAction.php) · [UpdateSerialIntakeAction](../../syilex/app/Actions/SerialIntake/UpdateSerialIntakeAction.php) · trait [HandlesSerialUnits](../../syilex/app/Actions/SerialIntake/Concerns/HandlesSerialUnits.php).
- API: `GET/POST /api/v1/serial-intakes`, `GET/PUT/DELETE /{ulid}`, `POST /{ulid}/approve`.

### Frontend
- **Input Pembelian Serial** (menu **Pembelian** → Pembelian Serial, route `/app/inventory/serial-intake`): form pilih produk serial (filter `is_serial`) + gudang + **supplier wajib**; tabel unit baris-per-baris (SN + modal + jual + grade/baterai/health/akun). `dataKey="_uid"` (key stabil, cegah input kehilangan fokus). Mode **edit** untuk draft. Header harus terisi sebelum **Tambah Baris** unit.
...
Header intake punya **Diskon Header (3 line)**, **Biaya Tambahan** (kirim + lain), **Pajak (DPP/PPN)**, **Pembulatan**, **Grand Total**, **Tempo**, **Catatan** — identik dengan PO karena **memakai ulang** [`PurchaseOrderCalculationService`](../../syilex/app/Services/PurchaseOrderCalculationService.php) (DRY): tiap unit dipetakan jadi 1 "detail" (qty 1, harga = modal).
...
- Helper: trait [`HandlesSerialUnits`](../../syilex/app/Actions/SerialIntake/Concerns/HandlesSerialUnits.php) (`calculateFinance` + `financialColumns`).
...
- **Field bisa dikoreksi:** `serial_number`, `harga_jual`, `grade`, `battery_condition`, `battery_health`, `battery_cycle_count`, `account_status`, `catatan`.
...
- **Form:** pilih produk serial via `ProductUnitPickerDrawer` (search server-side `GET /produks?is_serial=1&search=`, sama PBS) → muat unit tersedia → **centang** yang dikoreksi + edit per-unit + **bulk "Harga Jual (semua)"**. Jangan `Select` filter client: `getPerPage` max 100, produk di luar halaman pertama tidak muncul.
- Tabel `doc_serial_change` + `doc_serial_change_detail`. Prefix **PDS**. Actions [Create](../../syilex/app/Actions/SerialChange/CreateSerialChangeAction.php)/[Update](../../syilex/app/Actions/SerialChange/UpdateSerialChangeAction.php)/[Approve](../../syilex/app/Actions/SerialChange/ApproveSerialChangeAction.php) + trait [HandlesSerialChangeUnits](../../syilex/app/Actions/SerialChange/Concerns/HandlesSerialChangeUnits.php).
...
- **Register Unit Serial** — menu grup **Inventory** (read-only). Daftar tiap unit fisik lintas produk: `kode_internal`, `serial_number`, `harga_modal`, `harga_jual`, atribut, **status** (tersedia/terjual), **asal dokumen** (intake), gudang, waktu terjual. Filter: produk / gudang / status / cari (kode internal / SN). Ringkasan **total / tersedia / terjual** (global, tak terpengaruh filter status). **Export PDF** daftar. Asal dokumen **bisa diklik → buka dokumen Pembelian Serial**. **Nota Jual** hormati `doc_sales.source`: `manual` → Penjualan BO (`?detail=` + DetailDialog, gate `sales.view`); `pos` → `struk-online/:ulid` (public receipt).
  - API: `GET /api/v1/serial-units` (paginated + `summary`; eager `sale` termasuk `source`). Permission baca **reuse `serial-intake.view`**. **Kolom cost (`harga_modal` + `cost_per_unit`) di-gate `stok.view_hpp`** — user tanpa izin HPP tak menerima field cost (backend `makeHidden`, FE sembunyikan kolom + PDF); `harga_jual` tetap tampil. **Export Excel/PDF** ikut strip kolom cost untuk yang tak berizin.
  - File: [SerialUnitController.php](../../syilex/app/Http/Controllers/Api/V1/SerialUnitController.php), [SerialUnitRegisterPage.vue](../../../syilex-frontend/src/views/inventory/SerialUnitRegisterPage.vue), route `inventory-serial-units`.
- **Kartu Stok** — baris `PURCHASE` produk serial menyertakan `source_doc` (ulid intake) di response → No. Dokumen **bisa diklik buka dokumen PBS** (auto-open lewat query `?detail={ulid}` di SerialIntakePage). Lihat [StockCardController.php](../../syilex/app/Http/Controllers/Api/V1/StockCardController.php) (resolve `serialIntakeMap`).
- **Hutang Supplier** — hutang ber-sumber intake serial (`serial_intake_id`) kini tampil benar di **Export PDF & Excel**: kolom **"No. Dokumen"** (PO atau PBS) + kolom **"Sumber"** (PO/Serial). Sebelumnya kolom hard-code "No. PO" → kosong untuk serial. Lihat [SupplierHutangExport.php](../../syilex/app/Exports/SupplierHutangExport.php).
...
Pembelian serial = pembelian nyata (stok + hutang) → WAJIB tercermin di laporan Pembelian. Sumber data terpadu [PurchaseReportSource](../../syilex/app/Services/PurchaseReportSource.php) meng-UNION `doc_purchase_order(_detail)` + `doc_serial_intake`/`serial_units` (dinormalkan). Branch PO **identik** query lama → data tanpa serial = hasil sebelumnya.
...
- **Izin:** akses laporan pembelian = **`laporan.pembelian`** (per-kategori, lihat [ai/backend.md](../ai/backend.md) §Permission); nilai uang di-strip kecuali **`po.view_harga`**. **Laporan Diskon** isinya 100% nilai diskon → di-gate **penuh `po.view_harga`** (view + export 403 tanpa izin), menu Diskon disembunyikan tanpa `po.view_harga`.
...
- **Import Master** — kolom `is_serial` + auto-scaffold produk serial ([ImportController](../../syilex/app/Http/Controllers/Api/V1/ImportController.php)). **Tidak membuat stok/unit** (kolom impor hanya `minimum_stok`, bukan stok awal → unit hanya lahir via Pembelian Serial, tak ada stok hantu). **`is_serial` IMMUTABLE pada upsert** — baris yang mencoba flip status serial produk existing ditolak (skipped + error) untuk cegah desync. Operasi inventory serial (transfer/adjustment/opname/retur) tidak lewat import.
...
Karena SN boleh kembar, scan/ketik di POS & picker memakai **scan pintar** lewat `GET /serial-units/lookup?code=&warehouse_id=` ([SerialUnitController::lookup](../../syilex/app/Http/Controllers/Api/V1/SerialUnitController.php)):
...
- **Status unit baru:** `hilang` (opname), `retur` (retur beli). Kolom `status` string (tanpa migrasi enum). Validasi unit terpilih: trait [ResolvesSelectedUnits](../../syilex/app/Actions/Serial/Concerns/ResolvesSelectedUnits.php) (milik produk + di gudang sumber + `tersedia` + count==qty).
...
### Per modul
- **Transfer** — unit terpilih pindah `warehouse_id` (status tetap `tersedia`), 2 movement (TRANSFER_OUT/IN). [ApproveTransferAction](../../syilex/app/Actions/Transfer/ApproveTransferAction.php).
- **Adjustment** — **keluar (kredit)**: pilih unit → status **per-unit pilihan user `rusak`/`hilang`** (default `rusak`; map JSON `serial_unit_statuses` di detail) + movement OUT, stock_card cost = `cost_per_unit`. Bila `source=opname` → semua `hilang` (abaikan map). **Masuk (debit) DILARANG** (unit lahir hanya via Pembelian Serial). [ApproveAdjustmentAction](../../syilex/app/Actions/Adjustment/ApproveAdjustmentAction.php).
- **Stock Opname** — checklist SN **hadir** (`serial_unit_ids_present`, default semua tercentang di form); SN tak hadir → selisih kurang → unit `hilang` via adjustment turunan (`source=opname`). Selisih **lebih** untuk serial **ditolak**. [ApproveStockOpnameAction](../../syilex/app/Actions/StockOpname/ApproveStockOpnameAction.php).
- **Retur Beli** — 1 baris per produk serial; **harga retur = rata-rata `harga_modal` unit** (subtotal = Σ modal = kredit supplier), valuasi stok keluar = rata-rata `cost_per_unit` (landed). Saat **lock**: unit → status `retur` + movement OUT. Trait [PreparesSerialReturnDetails](../../syilex/app/Actions/PurchaseReturn/Concerns/PreparesSerialReturnDetails.php), [LockPurchaseReturnAction](../../syilex/app/Actions/PurchaseReturn/LockPurchaseReturnAction.php).
  - **Link PBS (opsional):** header `serial_intake_id` (XOR dengan `po_id`). Prefill 1 produk + unit `tersedia` dari intake; picker di-scope `intake_id`. Approve men-net `SupplierHutang` milik PBS itu (bukan FIFO). Mode bebas tetap FIFO semua hutang supplier.
- **Retur Jual BO** — `SerialUnitPicker` dengan `status=terjual` (+ filter `sales_id` / `sale_detail_id` / `customer_id`). Mode dokumen: revert unit nota. Mode bebas: unit terjual customer → `tersedia` + stok IN. Endpoint `available` mengizinkan `retur-jual.*`.
...
### Integritas
- `php artisan data:verify` invariant serial: `COUNT(serial_units 'tersedia' per produk+gudang) == inventory_stock.qty` + integritas terjual/sale_id. [VerifyDataInvariants](../../syilex/app/Console/Commands/VerifyDataInvariants.php).
...
- **Pilih produk:** `ProductUnitPickerDrawer` + search server (bukan Select 100 pertama). Produk immutable saat edit draft.
...
- Tabel `doc_serial_hpp_correction(_detail)`. [Actions](../../syilex/app/Actions/SerialHppCorrection/) + [Controller](../../syilex/app/Http/Controllers/Api/V1/SerialHppCorrectionController.php). FE: [SerialHppCorrectionPage.vue](../../../syilex-frontend/src/views/master/SerialHppCorrectionPage.vue) + [Form](../../../syilex-frontend/src/views/master/SerialHppCorrectionFormPage.vue).

## 4.13 Dashboard
Pending-approval dashboard kini memuat **serial-intake, serial-change, serial-hpp** (hitung draft, hormati permission `*.approve`). [DashboardController](../../syilex/app/Http/Controllers/Api/V1/DashboardController.php). Saat **Modul Elektronik OFF** (§4.14) ketiganya dibuang dari `$approvalModules` → tak muncul di dashboard.
...
- **Helper:** [`SettingService::isElektronikEnabled()`](../../syilex/app/Services/SettingService.php) — **default TRUE** bila baris setting belum ada (instalasi lama / lingkungan tes → fitur serial existing tetap jalan). Seed: migration `2026_06_19_100000_add_elektronik_module_setting` (idempotent) + [SettingSeeder](../../syilex/database/seeders/SettingSeeder.php).
- **Backend gate (saat OFF):**
  - Middleware [`feature.elektronik`](../../syilex/app/Http/Middleware/EnsureElektronikEnabled.php) (alias di `bootstrap/app.php`) → **403** di 4 grup route serial (`serial-intakes`, `serial-changes`, `serial-units`, `serial-hpp-corrections`) **+** PBS list/returnable di `purchase-returns/serial-intake*`.
  - [`ProdukController::store`](../../syilex/app/Http/Controllers/Api/V1/ProdukController.php) & [`ImportController`](../../syilex/app/Http/Controllers/Api/V1/ImportController.php) tolak `is_serial=true`; template import omit kolom `is_serial`.
  - Product-search POS/Transfer/Adj/Opname/Retur + mutasi `serial_unit_ids` / `serial_intake_id` → filter/422.
  - [`DashboardController`](../../syilex/app/Http/Controllers/Api/V1/DashboardController.php) buang kartu approval serial.
  - Role matrix: sembunyikan modul `serial-*`; update role **preserve** permission serial existing di DB.
- **Frontend gate (saat OFF):** getter `useSettingsStore().serialEnabled` → sembunyikan 4 menu serial (AppMenu), guard route (`meta.requiresElektronik`), checkbox Produk Serial, POS SN picker + scan-lookup, PBS di retur beli, `SerialUnitPicker` self-gate, filter sumber Serial di laporan pembelian, tip Print Barcode, badge SERIAL di stok.
- **Lock disable:** modul **tak bisa dimatikan** selama masih ada produk/unit serial **atau** dokumen PBS / draft serial-change / draft serial-hpp — guard di `SettingController` + `GET settings/elektronik-lock`.
- **Reset Database:** chip wipe serial **tetap tampil** saat OFF (agar bisa bersihkan sisa data).
- **Test:** [ElektronikModuleTest](../../tests/Feature/Settings/ElektronikModuleTest.php), [RoleSerialPermissionTest](../../tests/Feature/Role/RoleSerialPermissionTest.php).

### 4.15 Mode bebas retur (setting)

- `returns.sales_allow_free` / `returns.purchase_allow_free` (boolean, **default true**) di Pengaturan → Stok & Kalkulasi → section Retur.
- Saat OFF: create retur BO wajib dokumen (nota / PO|PBS); draft free preexisting tetap boleh dilock/approve.
- Helpers: `SettingService::isSalesReturnFreeAllowed()` / `isPurchaseReturnFreeAllowed()`. Runtime: `GET settings/runtime` group `returns`.
- Test: [SalesReturnFreeModeSettingTest](../../tests/Feature/Sales/SalesReturnFreeModeSettingTest.php).
...
- **Void** ([VoidSalesAction](app/Actions/Sales/VoidSalesAction.php)): semua unit nota → `tersedia` + movement `IN`, avg di-recompute (**Metode A**). **Retur POS** ([ProcessSalesReturnAction](app/Actions/Sales/ProcessSalesReturnAction.php)): pilih SN yang kembali (jumlah=qty) → `tersedia` + Metode A. **Retur BO** ([LockSalesReturnAction](app/Actions/SalesReturn/LockSalesReturnAction.php)): sama — revert unit + Metode A; nilai retur linked = pool POS (`total_setelah_diskon × (1+pajak%)`, tanpa biaya/pembulatan nota). Trait DRY [`RevertsSerialUnits`](app/Actions/Sales/Concerns/RevertsSerialUnits.php).
