> **Status:** canonical
> **SSoT kode:** syilex/app/ · syilex/routes/ · syilex-frontend/src/
...
- Print thermal via **browser Web API** (Chrome/Edge); gagal/unpaired → PDF fallback
...
sequenceDiagram
    ...
    alt Browser transport gagal / belum pair
...
sequenceDiagram
    ...
    Note over BE: Non-serial: HPP tidak weighted-recalc<br/>Serial: Metode A (rata unit tersedia)
...
- Retur jual BO (`source=manual`): mode bebas (tanpa nota) digate setting `returns.sales_allow_free` (default true). Linked: pool nilai seperti POS (`total_setelah_diskon × (1+pajak%)`, tanpa biaya/pembulatan nota).
- Retur beli: mode bebas digate `returns.purchase_allow_free`; PBS link digate Modul Elektronik.

---

## 3B. Feature toggles (settings)

```mermaid
flowchart TD
  settings[Pengaturan]
  elektronik[modules.elektronik_enabled]
  salesFree[returns.sales_allow_free]
  purchaseFree[returns.purchase_allow_free]
  settings --> elektronik
  settings --> salesFree
  settings --> purchaseFree
  elektronik -->|OFF| hideSerial[Hide menu serial PBS picker SN UX]
  elektronik -->|OFF| blockSerial[403 serial routes + 422 serial_unit_ids]
  salesFree -->|OFF| requireNota[Retur jual BO wajib nota]
  purchaseFree -->|OFF| requirePoPbs[Retur beli wajib PO atau PBS]
```

| Setting | Default | Efek OFF |
|---------|---------|----------|
| `modules.elektronik_enabled` | true | Seluruh surface serial hilang/diblok (lihat [docs/domain/serial.md §4.14](docs/domain/serial.md)) |
| `returns.sales_allow_free` | true | Create retur jual BO wajib `sales_id` |
| `returns.purchase_allow_free` | true | Create retur beli wajib `po_id` atau `serial_intake_id` |

Runtime expose: `GET /settings/runtime` (group `returns` + tax/rounding/…). Public: `GET /settings/public` (group `modules`).

---

## 4. Price Change Auto-Apply (Laravel Schedule)

```mermaid
graph TD
    A[OS cron / Laragon schedule:run] --> B[Artisan price-change:apply]
    B -->|every 5 min withoutOverlapping| C[ApplyPriceChangeAction lockForUpdate header]
    C -->|Lock taken| Z[Exit]
    C -->|Acquired| D[Query pending docs tanggal_berlaku <= now]
    D -->|Empty| Z
    D -->|Ada batch| E[Loop ApplyPriceChangeAction]
    E --> F[UPDATE master_produk.harga_*]
    F --> Z
```

Jadwal di `syilex/routes/console.php`. Production **butuh** `php artisan schedule:run` (cron). Bukan middleware API.
...
graph TB
    ...
    subgraph "HTTP Layer"
        MW[Middleware<br/>Auth, Idempotency,<br/>SecurityHeaders, feature.elektronik]
...
3. HPP weighted-avg non-serial pada approve PO/PBS, ADJUSTMENT_IN, dan Repack. Serial: Metode A pada jual/void/retur.
...
graph LR
    ...
    C --> D[CleanupActivityLog<br/>API group]
    D --> E[PreventApiCaching]
    E --> F[SecurityHeaders]
    F --> G[feature.elektronik<br/>serial routes]
    G --> H[Controller Action]
    H --> I[Response]
```

Price change **tidak** ada di pipeline API — lihat §4 (Artisan schedule).
...
- [`../ai/README.md`](../ai/README.md) — AI agent guide
- [`api.md`](api.md) — endpoint index
- [`../ops/deploy.md`](../ops/deploy.md) — production deploy
- [`../ops/restore-drill.md`](../ops/restore-drill.md) — disaster recovery
- [`../00-start/onboarding.md`](../00-start/onboarding.md) — new dev setup
- Actions: `syilex/app/Actions/` — baca source per flow
