> **Status:** canonical (index); detail routes di kode
> **SSoT kode:** syilex/routes/api.php
> **Jika konflik:** ikuti kode, lalu update dokumen ini.
# POSIP API Documentation

> **Catatan:** dokumen ini = **index** endpoint. Detail otoritatif: `syilex/routes/api.php` + FormRequest/Controller.
...
```
Ekspos store, currency, regional, text, number, **modules** (`elektronik_enabled`). Dipakai FE untuk `serialEnabled` tanpa auth settings.

### Runtime settings (auth, tanpa `settings.view`)
```
...
|---|---|---|
| GET/POST | `/produks` | `produk.view` / `produk.create` |
| GET/PUT/DELETE | `/produks/{ulid}` | `produk.view` / `produk.update` / `produk.delete` |
| GET | `/brands`, `/tipes`, `/kategoris`, `/grups` | `{entity}.view` |
| GET | `/customers`, `/suppliers` | `{entity}.view` |
| GET | `/metode-pembayarans` | `metode-bayar.view` |
| GET | `/warehouses`, `/pos-terminals` | `{entity}.view` |

Setiap master data punya pola: `index`, `show`, `store`, `update`, `destroy`, `list` (dropdown), `export` (excel). Nama resource **plural** di route (`/produks`, bukan `/produk`).

### Penjualan BO / AR (ringkas)
| Method | Endpoint | Permission (contoh) |
|---|---|---|
| CRUD + approve/void | `/sales` | `sales.*` |
| CRUD + lock/approve | `/sales-returns` | `retur-jual.*` |
| List / aging | `/customer-piutangs` | `piutang.*` |
| CRUD | `/customer-deposits` | `deposit-customer.*` |
| CRUD + complete | `/pembayaran-piutangs` | `pembayaran-piutang.*` |

### Serial / Elektronik (ringkas; butuh `modules.elektronik_enabled`)
| Method | Endpoint | Permission |
|---|---|---|
| Intake draft→approve | `/serial-intakes` | `serial-intake.*` |
| Register / lookup | `/serial-units` | `serial-intake.view` (+ harga sensitif) |
| Perubahan data | `/serial-changes` | `serial-change.*` |
| Koreksi HPP serial | `/serial-hpp-corrections` | `serial-hpp.*` |

Middleware `feature.elektronik` pada grup route serial. Detail: [`docs/domain/serial.md`](serial.md).
...
#### Penjualan â€” Per Nota (`/sales-report`)
| Method | Endpoint | Permission |
|---|---|---|
...
#### Penjualan â€” Per Barang (`/sales-product-report`)
| Method | Endpoint | Permission |
|---|---|---|
...
#### Penjualan â€” Financial (`/sales-financial-report`)
| Method | Endpoint | Permission |
|---|---|---|
...
#### Laporan Keuangan / Analitik (Sprint 1â€“3)

**Permissions (granular â€” `laporan.view` hanya untuk widget dashboard, BUKAN halaman laporan):**
...
#### Keuangan â€” Gross Profit (`/reports/gross-profit`)
| Method | Endpoint | Permission |
|---|---|---|
...
### Backup Download (Updated â€” ZIP Format)
```
    ...
    "items": [
        {
            ...
            "qty": 3,
            "harga": 10000,
            "diskon_5_tipe": "nominal",  // â† hanya diskon_5 (manual kasir) yang di-trust dari FE
            "diskon_5_nilai": 500
        }
    ],
    "payments": [ ... ]
