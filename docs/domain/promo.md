> **Status:** canonical
> **SSoT kode:** syilex/app/Services/Promo* · PromoController · PromoFormPage
> **Jika konflik:** ikuti kode, lalu update dokumen ini.

# Modul Promosi — Desain Detail

> Ringkas agent: anti-fraud checkout di [`../ai/business-rules.md`](../ai/business-rules.md) §A.  
> Dokumen ini panjang — gunakan heading sebagai TOC; verifikasi perilaku di kode sebelum mengubah.

## TOC cepat

- Konsep Inti
- Database Schema (`doc_promo`, details)
- Matching / aplikasi diskon
- API & permission
- Frontend form
...
→ Berbeda dengan Price Change yang di-apply via Artisan cron (`price-change:apply`), bukan login listener
```
