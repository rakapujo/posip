...
class UpdatePriceChangeAction
{
     ...
     * Execute the action.
     */
    public function execute(DocPriceChange $priceChange, array $data): DocPriceChange
    {
        $this->ensureAuthenticated();

        if (!$priceChange->isDraft()) {
            throw ValidationException::withMessages([
                'status' => ['Hanya dokumen dengan status draft yang dapat diedit.'],
            ]);
        }

        return DB::transaction(function () use ($priceChange, $data) {
            // Check for locked products (products in OTHER drafts/scheduled, not this one)
            $productIds = collect($data['details'])->pluck('product_id');
            $lockedProducts = $this->getLockedProductIds($priceChange->id);
            ...
            if ($conflictingProducts->isNotEmpty()) {
                ...
                throw ValidationException::withMessages([
                    'details' => ["Produk berikut sudah ada di dokumen perubahan harga lain: {$productNames}"],
                ]);
            }
    ...
    private function createDetail(DocPriceChange $priceChange, array $detail, string $priceMode): void
    {
        ...
        // Capture current prices (re-capture at update time)
    ...
    /**
     * Get IDs of products that are locked in other drafts/scheduled (excluding current document).
     */
    private function getLockedProductIds(int $excludeId): array
    {
        return DocPriceChangeDetail::whereHas('priceChange', function ($query) use ($excludeId) {
