...
/**
 * Get primary color preset extension
 */
function getPrimaryPresetExt(primaryName) {
...
/**
 * Apply full theme based on preferences
 * @param {Object} preferences
 * @param {string} preferences.preset - Preset name (Aura, Lara, Nora)
 * @param {string} preferences.primary - Primary color name
 * @param {string} preferences.surface - Surface color name
 */
export function applyFullTheme(preferences) {
    const { preset = 'Lara', primary = 'blue', surface = 'slate' } = preferences;

    const presetValue = presets[preset];
    const surfacePalette = surfaceColors[surface];
    const primaryExt = getPrimaryPresetExt(primary);

    if (presetValue && surfacePalette) {
        $t().preset(presetValue).preset(primaryExt).surfacePalette(surfacePalette).use({ useDefaultOptions: true });
    }
}
...
export function applyPrimaryColor(primaryName) {
    const primaryExt = getPrimaryPresetExt(primaryName);
    updatePreset(primaryExt);
}

/**
 * Apply surface color only
 * @param {string} surfaceName
 */
export function applySurfaceColor(surfaceName) {
    const surfacePalette = surfaceColors[surfaceName];
    if (surfacePalette) {
 ...
 */
export function loadPreferencesFromCache() {
    try {
