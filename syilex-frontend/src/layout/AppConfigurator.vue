...
const { layoutConfig, isDarkTheme, changeMenuMode, changePreset, changePrimary, changeSurface } = useLayout();
...
function updateColors(type, color) {
    if (type === 'primary') {
        // Update via layout composable (syncs to preferences store)
        changePrimary(color.name);
    } else if (type === 'surface') {
        // Update via layout composable (syncs to preferences store)
        changeSurface(color.name);
    }
