...
// Permissions
const canView = computed(() => authStore.can('deposit-supplier.view'));
...
const summaryItems = computed(() => [
    { label: 'Total Deposit', value: canViewNominal.value ? formatCurrency(summary.value.total_deposit) : '-', tone: 'info' },
    { label: 'Sudah Terpakai', value: canViewNominal.value ? formatCurrency(summary.value.total_used) : '-', tone: 'orange' },
    { label: 'Sisa Deposit', value: canViewNominal.value ? formatCurrency(summary.value.total_balance) : '-', tone: 'success' },
...
const formAllowsSupplierChange = ref(true);
const lastFormSupplierId = ref(null);

const canChangeSupplier = computed(() => formMode.value === 'create' || formAllowsSupplierChange.value);
...
// Form functions
function openCreateForm(supplierId = null) {
    formMode.value = 'create';
    editUlid.value = null;
    formAllowsSupplierChange.value = true;
    formData.value = {
    ...
    lastFormSupplierId.value = supplierId;
    formDialog.value = true;
}

function openCreateFromDetail() {
    openCreateForm(detailData.value.supplier?.id ?? null);
    detailDialog.value = false;
}

function onFormSupplierChange() {
    const newId = formData.value.supplier_id;
    if (lastFormSupplierId.value == null) {
        lastFormSupplierId.value = newId;
        return;
    }
    if (newId === lastFormSupplierId.value) return;

    confirm.require({
        message: 'Ganti supplier akan mengosongkan nominal, no. referensi, dan keterangan. Lanjutkan?',
        header: 'Konfirmasi Ganti Supplier',
        icon: 'pi pi-exclamation-triangle',
        rejectLabel: 'Batal',
        acceptLabel: 'Lanjutkan',
        rejectClass: 'p-button-secondary p-button-outlined',
        accept: () => {
            formData.value.nominal_awal = 0;
            formData.value.no_referensi = '';
            formData.value.keterangan = '';
            lastFormSupplierId.value = newId;
        },
        reject: () => {
            formData.value.supplier_id = lastFormSupplierId.value;
        }
    });
}
...
async function openEditForm(data) {
    ...
    try {
        ...
        if (response.data.success) {
            ...
            formAllowsSupplierChange.value = deposit.can_edit === true;
            formData.value = {
            ...
            lastFormSupplierId.value = formData.value.supplier_id;
            formDialog.value = true;
        }
    } catch (error) {
...
<template>
    <div class="card">
        <MoneySummaryPanel title="Ringkasan Deposit Supplier" :items="summaryItems" :cols="5" />
            ...
            <Column header="Aksi" style="min-width: 150px" alignFrozen="right" frozen>
                <template #body="{ data }">
                    <RowActionButtons>
                        ...
                        <Button v-if="canUpdate && (data.can_edit ?? isManual(data))" icon="pi pi-pencil" severity="warning" text rounded @click="openEditForm(data)" v-tooltip.top="'Edit'"  />
                        <Button v-if="canDelete && (data.can_delete ?? (isManual(data) && data.nominal_terpakai == 0))" icon="pi pi-trash" severity="danger" text rounded @click="confirmDelete(data)" v-tooltip.top="'Hapus'"  />
                    </RowActionButtons>
                </template>
            </Column>
        </DataTable>
        ...
        >
                                        ...
                                        <Column header="No. Pembayaran" style="min-width: 160px">
                                            <template #body="{ data }">
                                                <RouterLink
                                                    ...
                                                    :to="{ name: 'pembelian-pembayaran-hutang-edit', params: { ulid: data.pembayaran_ulid } }"
            ...
            <template #footer-extra>
                <div class="flex flex-wrap gap-2">
                    ...
                    <Button v-if="canUpdate && detailData.can_edit"
                        label="Edit"
                        icon="pi pi-pencil"
                        severity="warning"
                        @click="
                            ...
                            detailDialog = false;
                        " />
                    <Button v-if="canDelete && detailData.can_delete"
                        label="Hapus"
                        icon="pi pi-trash"
                        severity="danger"
                        @click="
        ...
        <Dialog v-model:visible="formDialog" :header="formDialogTitle" modal :style="{ width: '500px' }" :closable="!formLoading">
            <div class="space-y-4">
                ...
                <div>
                    ...
                    <Select v-model="formData.supplier_id" :options="suppliers" optionLabel="nama_supplier" optionValue="id" placeholder="Pilih Supplier" class="w-full" filter :disabled="formMode === 'edit' && !canChangeSupplier" @change="onFormSupplierChange" />
                </div>
