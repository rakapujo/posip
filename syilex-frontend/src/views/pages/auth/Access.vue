<script setup>
/* anchor: Login.vue form-inner panel + empty-state floor; diverge: access-denied mark */
...
<template>
    <div class="min-h-screen flex items-center justify-center bg-surface-100 px-5 py-8 md:px-8 md:py-12">
        <div class="w-full max-w-sm md:max-w-md bg-surface-0 border border-surface-200 rounded-2xl shadow-sm px-6 py-8 md:px-8 md:py-10 text-center">
            ...
            />
            <div class="inline-flex items-center justify-center w-12 h-12 rounded-full border border-surface-300 text-orange-500 mt-6">
                <i class="pi pi-lock text-2xl" />
            </div>
            <p class="text-surface-900 font-semibold text-lg mt-3 mb-0">Akses ditolak</p>
            <p class="text-surface-500 text-sm leading-relaxed mt-2 mb-0">Anda tidak punya izin untuk halaman ini. Hubungi admin.</p>
            <Button as="router-link" :to="homeTo" :label="homeLabel" severity="warn" class="w-full !min-h-11 mt-7" />
