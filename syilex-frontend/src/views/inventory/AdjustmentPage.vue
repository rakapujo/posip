...
function sourceLabel(source) {
    return source === 'opname' ? 'Opname' : 'Manual';
}

function sourceSeverity(source) {
    return source === 'opname' ? 'info' : 'secondary';
}
...
<template>
    <div class="card">
        <Toolbar class="mb-6">
            ...
            <template #end>
                <ListFiltersSheet :active-count="activeFilterCount">
                    <Select v-model="additionalFilters.warehouse_id" :options="warehouses" optionLabel="nama_warehouse" optionValue="id" placeholder="Semua Gudang" filter showClear @change="onFilter" />
        ...
        >
            ...
            <Column header="Sumber" style="min-width: 110px">
                <template #body="{ data }">
                    <Tag :value="sourceLabel(data.source)" :severity="sourceSeverity(data.source)" />
                </template>
            </Column>
        ...
        >
            <template #content>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    ...
                    <DetailItem label="Sumber" :value="sourceLabel(detailData.source)" type="badge" :badge-severity="sourceSeverity(detailData.source)" />
                    <div v-if="detailData.source === 'opname' && detailData.opname">
                        <span class="text-surface-500 text-sm block mb-1">Stock Opname</span>
                        <router-link
                            :to="{ name: 'inventory-opname' }"
                            class="text-primary font-medium hover:underline"
                            @click="detailDialog = false"
                        >
                            {{ detailData.opname.nomor_dokumen }}
                        </router-link>
                    </div>
                </div>
